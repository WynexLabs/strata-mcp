...
const inputShape = {
  weights: z
    .array(z.number())
    .min(1)
    .describe("Portfolio weights (sum ≈ 1.0, ±0.01). Length N defines the portfolio."),
  annualizedReturns: z
    .array(z.number())
    .min(1)
    .describe("Annualized expected returns per asset (decimal), length must equal weights.length."),
  covarianceMatrix: z
    .array(z.array(z.number()))
    .describe("N×N annualized return covariance matrix, where N = weights.length."),
